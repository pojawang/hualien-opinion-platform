...
export function normalizeArticle(item) {
  ...
  return {
    ...
    view_count: Number.isFinite(Number(item.view_count)) ? Math.max(0, Math.trunc(Number(item.view_count))) : 0,
    ...
    hotness_score: Number.isFinite(Number(item.hotness_score)) ? Math.max(0, Number(item.hotness_score)) : 0,
    analysis_keywords: Array.isArray(item.analysis_keywords) ? item.analysis_keywords.slice(0, 10) : [],
    ai_analyzed: Boolean(item.ai_analyzed),
    ...
    review_text: cleanText(item.review_text || '') || null,
    place_type: cleanText(item.place_type || '') || null,
